...
function OverviewUserItem({ user }) {
  return (
    <article className="admin-item">
      ...
      <p className="admin-item__footer">Joined {formatDateTime(user.createdAt)}</p>
    </article>
  );
}
  ...
  return (
    <article className="admin-item">
      ...
      <p className="admin-item__footer">
        {product.category?.name || "General"} | {formatDateTime(product.createdAt)}
      </p>
...
function NotificationItem({ notification, onMarkRead }) {
  ...
  return (
    <article className="admin-item">
      ...
      {contextText ? <p className="admin-item__summary">{contextText}</p> : null}
      ...
      <div className="admin-item__actions">
        ...
        {needsReview ? (
          ...
          <button
            ...
            className="btn btn--secondary btn--small"
            onClick={() =>
              onChangeStatus(product.id, isUnlistedApprovedListing ? "available" : "suspended")
            }
          >
...
export function AdminDashboardPage({ page }) {
  ...
  const [productsFilters, setProductsFilters] = useState(INITIAL_PRODUCTS_FILTERS);
  ...
  async function loadCategories() {
    ...
    if (!result.ok || !result.data?.success) {
      showMessage(result.data?.message || "Failed to load categories.", "error");
      return;
    }
  ...
  async function handleUserToggleStatus(targetUser) {
    ...
    const reason = window.prompt(
      `${isActive ? "Optional activation note" : "Optional suspension reason"} for ${targetUser.name}:`,
    );

    const result = await fetchApi(`/api/v1/admin/users/${targetUser.id}/status`, {
      method: "PUT",
      auth: true,
      body: {
        isActive,
  ...
  async function moderateProduct(productId, action) {
    const reasonInput = window.prompt(
      `${action === "approve" ? "Optional approval note" : "Required note for the owner"}:`,
    );
    if (reasonInput === null) {
      return;
    }
    ...
    <SiteLayout page={page} user={user} onLogout={logout}>
      <section className="page-hero admin-hero">
        <div>
          ...
          <p>
            Review platform health, moderate users and products, inspect rentals,
            and manage categories from one page with the same multi-page React
            architecture as the rest of the front end.
          </p>
        </div>
      ...
      <section className="admin-layout">
        ...
        <section className="admin-content">
          ...
          <section className={`admin-panel${activePanel === "overview" ? " is-active" : ""}`}>
            ...
            <div className="admin-grid">
              <article className="surface-panel">
                <SectionHeading eyebrow="Recent users" title="Newest accounts" compact />
                <AdminList
                  items={overview?.recent?.users || []}
                  emptyMessage="No recent users found."
                  renderItem={(item) => <OverviewUserItem key={item.id} user={item} />}
                />
              </article>
            ...
            <article className="surface-panel">
              <SectionHeading eyebrow="Recent rentals" title="Latest booking activity" compact />
              <AdminList
                items={overview?.recent?.rentals || []}
                emptyMessage="No recent rentals found."
                renderItem={(item) => <OverviewRentalItem key={item.id} rental={item} />}
              />
            </article>
          </section>
          ...
          <section className={`admin-panel${activePanel === "users" ? " is-active" : ""}`}>
            <article className="surface-panel">
              <form
                className="form-grid admin-filters"
                onSubmit={async (event) => {
          ...
          <section className={`admin-panel${activePanel === "rentals" ? " is-active" : ""}`}>
            <article className="surface-panel">
              <form
                className="form-grid admin-filters"
                onSubmit={async (event) => {
            ...
            <article className="surface-panel">
              ...
              <AdminList
                ...
                renderItem={(item) => <RentalItem key={item.id} rental={item} />}
              />
            </article>
          </section>

          <section className={`admin-panel${activePanel === "reports" ? " is-active" : ""}`}>
            <article className="surface-panel">
              <form
                className="form-grid admin-filters"
                onSubmit={async (event) => {
            ...
            <div className="admin-grid">
              <article className="surface-panel">
                <SectionHeading eyebrow="Distributions" title="Platform composition" compact />
              ...
              <article className="surface-panel">
                <SectionHeading eyebrow="Trend lines" title="Growth by month" compact />
                <div className="list-stack">
                  {report ? (
                    <>
                      <TrendGroup
            ...
            <div className="admin-grid">
              <article className="surface-panel">
                <SectionHeading eyebrow="Categories" title="Top listing categories" compact />
                <AdminList
                  ...
                  renderItem={(item) => (
                    <article className="admin-item" key={item.id}>
                      <div className="admin-item__header">
                        ...
                        <span className="tag tag--light">{item.totalProducts}</span>
                      </div>
                      <p className="admin-item__summary">
              ...
              <article className="surface-panel">
                <SectionHeading eyebrow="Top products" title="Most rented listings" compact />
                <AdminList
                  ...
                  renderItem={(item) => (
                    <article className="admin-item" key={item.id}>
                      <div className="admin-item__header">
                        ...
                        <span className="tag tag--light">{item.totalRentals}</span>
                      </div>
                      <p className="admin-item__meta">
            ...
            <div className="admin-grid admin-grid--categories">
              <article className="surface-panel">
                <div className="section-heading section-heading--compact">
                  <div>
                    <p className="eyebrow">
                      {editingCategoryId ? "Category editor" : "Category studio"}
                    </p>
                    <h3>
              ...
              <article className="surface-panel">
                <SectionHeading eyebrow="Catalog" title="All categories" compact />
                <AdminList
                  items={categories}
