...
function scoreSimilarProduct(baseProduct, candidate) {
  ...
  const baseTags = new Set(
    (Array.isArray(baseProduct.tags) ? baseProduct.tags : []).map(
      normalizeText,
    ),
  );
  const candidateTags = (
  ...
  if (basePrice != null && candidatePrice != null) {
    const diffRatio =
      ...
      Math.max(basePrice, candidatePrice, 1);
...
async function getRecommendations(req, res) {
  ...
  try {
    ...
    let scoredProducts = await rankRecommendedProducts(
      candidates,
      profile,
      req.user.id,
    );
    ...
    const recommendations = scoredProducts
      ...
      .map((item) =>
        formatScoredProduct(item.product, item.score, item.reasons),
      );
