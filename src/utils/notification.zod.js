...
const notificationRentalIdParamSchema = zod.object({
  id: zod.string().trim().uuid("Valid rental id is required"),
});

const notificationScopeParamSchema = zod.object({
  scope: zod.enum(["notifications", "bookings", "rentals"]),
});
...
export default {
  ...
  notificationRentalIdParamSchema,
  notificationScopeParamSchema,
